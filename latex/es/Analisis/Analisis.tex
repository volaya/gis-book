...
En ocasiones, los resultados expresan \textbf{la misma variable} que el dato de partida (por ejemplo, el cálculo de una media), y en otros \textbf{las variables de entrada y salida son distintas} (por ejemplo, si a partir de una capa de elevaciones calculamos una de pendientes).
...
\section{Algunos ejemplos de análisis espacial}
...
Pueden combinarse con otros de los análisis disponibles, por ejemplo para la \textbf{selección} de una serie de elementos sobre los que posteriormente se desarrollara el análisis en cuestión.
...
\begin{figure}[!hbt]   
\centering
\includegraphics[width= \columnwidth]{Analisis/Interseccion.pdf}
...
Un caso particular, ya mencionado en un capítulo anterior, es la \textbf{conversión entre modelos de datos}, esto es, entre el modelo ráster y el vectorial. La figura \ref{Fig:Conversiones} muestra un ejemplo de esto. Partiendo de un mapa escaneado (una capa ráster) con curvas de nivel, pueden detectarse estas y crearse una capa vectorial con dichas curvas de nivel. Estas pueden posteriormente convertirse en un Modelo Digital de Elevaciones (ráster) mediante \textbf{interpolación}, el cual es posible convertir de nuevo en una capa vectorial de líneas.

\begin{figure}[!hbt]   
\centering
\includegraphics[width= \columnwidth]{Analisis/Conversiones.png}
